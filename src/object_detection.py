...
class ObjectDetection: # Object Detector
    def __init__(self, target_object_classes, model_file_path, media_source_path, conf_threshold):
        self.model = LoadModel(model_file_path).load_yolo_model()  # Load model with given path
        self.media_source = media_source_path  # Get media path # remove it so can use on other task
...
# make according
    ...
    def process_frame(self, frame):
        ...
        """
        detection_results = self.model(frame, verbose=False) # handle required object before return
        return detection_results

    def annotate_frame(self, frame, detection_results): # THERE'S SHOULD NOT BE
        """
        Draw bounding boxes only for the target objects.
        """
        if detection_results[0].boxes.data is not None: # make it for GPU and CPU
            ...
            for index, bounding_box in enumerate(bounding_boxes):
                ...
                if detected_class_id in self.target_class_ids and confidence_score >= self.conf_threshold:  # Filter detections by target class IDs
